...
pneumonia_model = load_model(os.path.join('static', 'model/Pneumonia-detection-model.h5'))
...
def Pneumonia(request):
    if request.method == "POST":
        ...
        input_prediction = pneumonia_model.predict(input_image_reshaped)
        ...
        if input_pred_label == 1:
            messages.success(request, 'The person does not have Peumonia')
            print('The person does not have Neumonia')
        else:
            messages.error(request, 'The person has Pneumonia')
            print('The person has Neumonia')



    return render(request, 'neumonia.html')